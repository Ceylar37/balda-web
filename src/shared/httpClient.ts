...
interface InterceptorResponseResult<T = any> {
...
type SuccessResponseFun<T = any> = (res: InterceptorResponseResult<T>, initialConfig: _RequestConfig) => any;
...
type FailureResponseFun = (e: ResponseError, initialConfig: _RequestConfig) => any;
...
export class HttpClient {
  ...
  readonly interceptors: {
    ...
    response: {
      use: <T>(onSuccess?: SuccessResponseFun<T>, onFailure?: FailureResponseFun) => ResponseInterceptor;
  ...
  private async runResponseInterceptors<T>(initialResponse: Response, initialConfig: _RequestConfig) {
    ...
    this.interceptorHandlers.response?.forEach(({ onSuccess, onFailure }) => {
      try {
        ...
        if (!onSuccess) return;
        body = onSuccess(response, initialConfig);
      } catch (error: any) {
        error.config = initialConfig;
        error.response = response;
        if (onFailure) {
          body = onFailure(error as ResponseError, initialConfig);
